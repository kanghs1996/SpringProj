...
public class ValidationController {
	...
	private boolean validate(FormCommand cmd,Model model) {
		...
		if(cmd.getGender() == null) {
			model.addAttribute("genderError", "성별을 선택 하세요");
			return false;
		}
		if(cmd.getInters() == null) {
			model.addAttribute("intersError", "관심사항을 선택 하세요");
			return false;
		}
		else {
			if(cmd.getInters().length < 2) {
				model.addAttribute("intersError", "관심사항은 2개이상 선택 하세요");
				return false;
			}
		}
		if(cmd.getGrade().length()==0) {
			model.addAttribute("gradeError", "학력을 선택 하세요");
			return false;
		}
		if(cmd.getSelf().trim().length()==0) {
			model.addAttribute("selfError", "자기소개를 입력 하세요");
			return false;
		}
		return true;
	}///////////////////////
